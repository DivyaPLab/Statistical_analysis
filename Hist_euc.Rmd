...
colors = c("#9E0000","#F66308","#F2CC8F","#A0785A","#582f0e","#ADC178","#099fb0","#BFD7EA", "#3D405B")
my_scale=scale_fill_manual(name=as.character(unique_sources), values = colors, na.translate=FALSE, drop=FALSE, limits = force)


# Saving the color palette for future use 
#saveRDS(palette, "/home/dpatel/sequences/euc/colors_class.rds")
...
Euc_Phylum=ggplot(Phylum_Filters, aes(x=Sample, weight=Abundance, fill=.data[[taxa_rank]])) +
  geom_bar() +
...
Euc_Phylum
ggsave("sequences/Euc/euc_Figures/Euc_Filtre_Phylum.pdf", width = 24, height = 12, plot = last_plot())
...
Phylum_Rocks$Sample <- factor(Phylum_Rocks$Sample, c("B1B","B5B","B7B","B8B","B3M","B4M","B1H","B6H","B7H",
                                                   "J1B","J3B","J7B","J8B","J2H","R5B","R7B","R2M","R3M","R7M","R1H"))


Euc_Phylum=ggplot(Phylum_Rocks, aes(x=Sample, weight=Abundance, fill=.data[[taxa_rank]])) +
  geom_bar() +
  labs(y ='Relative abundance (%)', paste(sample), x = "Roche", ) +
  scale_y_continuous(expand = c(0,0)) +
  theme(text = element_text(size = 20),axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  my_scale + 
  guides(fill=guide_legend(title="Phylum")) +
...
Euc_Phylum
ggsave("sequences/Euc/euc_Figures/Euc_Roche_Phylum.pdf", width = 24, height = 12, plot = last_plot())
