...
# Show the plot
p + xlim(-0.75, 0.45) + theme(text = element_text(size = 22), legend.text = element_text(size = 18))
ggsave("/home/dpatel/sequences/Euc/euc_Figures/PCOA_beta1_euc.tiff", width = 8, height = 6, plot = last_plot())
...
metadata <- metadata[metadata$Bioreacteur!= "Terrain", ]
...
Corr <- adonis2(all_dist ~ SampleType + Bioreacteur, data = meta_dist, by="margin")
print(Corr)
#             Df  SumOfSqs      R2       F    Pr(>F)    
# SampleType   1   1.1524    0.21057  9.2640  0.001 ***
# Bioreacteur  2   0.2790    0.05098  1.1213  0.348    
# Residual    33   4.1051    0.75010                  
# Total       36   5.4728    1.00000   
...
# So the days will be J1-J3, J5-J6 and J7-J12
```{r}
rm(list=ls())
...
meta13  <- metadata[metadata$Jour %in% c("2", "4", "6"), ]
meta56 <- metadata[metadata$Jour %in% c("10", "12"), ]
meta712 <- metadata[metadata$Jour %in% c("14", "16", "18", "20","22"), ]

# Now we have to join the metadata dataframes with the matrix and keep the lower half of the dist
meta13_dist <- inner_join(meta13, beta_dist, by = "Nom")
dist13 <- meta13_dist %>% select(all_of(meta13_dist$Nom)) %>% as.dist()

meta56_dist <- inner_join(meta56, beta_dist, by = "Nom")
dist56 <- meta56_dist %>% select(all_of(meta56_dist$Nom)) %>% as.dist()

meta712_dist <- inner_join(meta712, beta_dist, by = "Nom")
dist712 <- meta712_dist %>% select(all_of(meta712_dist$Nom)) %>% as.dist()


# Run the adonis2 function and look at the results
Corr13 <- adonis2(dist13 ~ Bioreacteur + Jour, data = meta13_dist, by="margin")
print(Corr13)
#              Df  SumOfSqs     R2      F     Pr(>F)    
# Bioreacteur  2   0.15636   0.25108  1.8664  0.211   
# Jour         1   0.35070   0.56314  8.3724  0.002 **
# Residual     3   0.12566   0.20178                 
# Total        6   0.62276   1.00000  

Corr56 <- adonis2(dist56 ~ Bioreacteur + Jour, data = meta56_dist, by="margin")
print(Corr56)
#              Df  SumOfSqs    R2       F     Pr(>F)  
# Bioreacteur  2   0.026509  0.47017  1.1583  0.4667
# Jour         1   0.014892  0.26413  1.3014  0.3000
# Residual     1   0.011443  0.20296              
# Total        4   0.056382  1.00000  

Corr712 <- adonis2(dist712 ~ Bioreacteur + Jour, data = meta712_dist, by="margin")
print(Corr712)
#             Df  SumOfSqs     R2       F    Pr(>F)    
# Bioreacteur  2  0.19739   0.29673  2.8440  0.052 . 
# Jour         1  0.23594   0.35469  6.7988  0.006 **
# Residual     6  0.20822   0.31301                 
# Total        9  0.66521   1.00000   
```
...
print(Corr)
#             Df  SumOfSqs     R2       F     Pr(>F)    
# Bioreacteur  2   0.1915   0.04872   0.9402  0.433    
# Jour         1   1.9035   0.48420  18.6873  0.001 ***
# Residual    18   1.8335   0.46639                   
# Total       21   3.9311   1.00000      
...
### Doing the PERMANOVA (ROCKS ONLY)
```{r}
set.seed(123)
# Remove the 'Terrain' data 
...
print(Corr)
#             Df  SumOfSqs     R2       F    Pr(>F)    
#Bioreacteur  2  0.21053 0.46479 5.2105  0.001 ***
#Residual    12  0.24243 0.53521                  
#Total       14  0.45296 1.00000                 
```
...
dbRDAfilter <-ggplot() +
  geom_hline(yintercept = 0, linetype = "solid", color = "gray") + #adding dashed lines on the y axis
...
dbRDAfilter
ggsave("/home/dpatel/sequences/Euc/euc_Figures/Euc_dbRDA_filter.jpeg", width = 6, height = 4, plot = last_plot())

```


### Doing a variance partitioning
```{r}

varpart_result <- varpart(ASV_table, ~ Moy_DIC, ~ pH, data = Meta_filters_all)


# Extract the components of interest
residual_variance <- varpart_result$varei$residual
fixed_effects_variance <- varpart_result$varei$fixed
random_effects_variance <- varpart_result$varei$random
proportion_variance_explained <- varpart_result$varpart

cat("Residual Variance:", residual_variance, "\n")
cat("Fixed Effects Variance:", fixed_effects_variance, "\n")
cat("Random Effects Variance:", random_effects_variance, "\n")
cat("Proportion of Variance Explained:", proportion_variance_explained, "\n")
...
ASV_table <- t(read.table("~/sequences/Euc/euc_csv/rarefied_asv.csv", header = TRUE, row.names = 1, sep = ","))
metadata <- read.table("~/sequences/Euc/euc_csv/rarefied_meta.csv", header=TRUE, sep=",", row.names = NULL)
mineral <-read.table("~/Mineralogie.csv", header=TRUE, sep=",", row.names = NULL)
...
# Since we want to see the correlation with the mineralogy and roughness, we'll keep the the initial model 
# (without the X.Undetermined because it has the same AIC than the initial model)
mod2 <- capscale(beta_dist ~  X.IronOxide +  X.RedFeldpath + X.WhiteFeldpath + X.Mica + Roughness, data = meta_mineral)
...
dbrdarock
ggsave("/home/dpatel/sequences/Euc/euc_Figures/Euc_dbRDA_rock.jpeg", width = 6, height = 4, plot = last_plot())
...
varpart_result <- varpart(ASV_table, ~ Roughness, ~X.Quartz, ~ X.RedFeldpath + X.WhiteFeldpath , ~ X.Mica , data = meta_mineral)
varpart_result
